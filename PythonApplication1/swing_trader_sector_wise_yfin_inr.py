...
# ─────────────────────────────────────────────────────────────────────────────
# INDIA SECTOR MAP — NSE sector indices (yfinance ^ prefix)
# ─────────────────────────────────────────────────────────────────────────────
INDIA_SECTOR_ETFS = {
    "Nifty 50":         "^NSEI",
    "Banking":          "^NSEBANK",
...
def get_india_sector_performance() -> pd.DataFrame:
    """Fetches today's % change for all India NSE sector indices."""
...
with tab_sectors:
    ...
    market_sel = st.radio(
        "Market", ["🇮🇳 India (NSE)"],
        horizontal=True, key="heatmap_market"
    )

    if market_sel == "🇮🇳 India (NSE)":
        st.caption("NSE Sector Indices · Refreshes every 15 min · Prices in INR")
        sector_df = get_india_sector_performance()
        nifty_row = sector_df[sector_df["ETF"] == "^NSEI"]
        if not nifty_row.empty:
            n50_pct   = nifty_row.iloc[0]["Today %"]
            n50_price = nifty_row.iloc[0]["Price"]
            col_n1, col_n2, col_n3 = st.columns(3)
            col_n1.metric("Nifty 50", f"₹{n50_price:,.0f}", f"{n50_pct:+.2f}%")
            col_n2.metric("India VIX", "–")
            col_n3.metric("Session", "NSE 09:15–15:30 IST")
        sector_df = sector_df[sector_df["ETF"] != "^NSEI"]  # remove index from tiles
    else:
    ...
    if sector_df.empty or "Today %" not in sector_df.columns:
        ...
        for _, row in sector_df.iterrows():
            ...
            price_sym = "₹" if market_sel == "🇮🇳 India (NSE)" else "$"
            html += (
                ...
                f"<div style='font-size:11px;opacity:.85'>5d: {fived:+.2f}%  ·  {price_sym}{row['Price']:,.0f}</div>"
                f"</div>"
            )
        html += "</div>"
...
with tab_etf:
    ...
    SG_ETF_UNIVERSE = {
        # ── India ETFs ────────────────────────────────────────────────────────
        # US-listed (available on IBKR — note: US estate tax risk if >USD 60k)
        "INDA":    {"Name": "iShares MSCI India ETF",           "Focus": "India Broad Market"},
        "INDY":    {"Name": "iShares India 50 ETF",             "Focus": "India Large Cap"},
        "SMIN":    {"Name": "iShares MSCI India Small-Cap",     "Focus": "India Small Cap"},
        "EPI":     {"Name": "WisdomTree India Earnings ETF",    "Focus": "India Earnings"},
        # NSE-listed India ETFs (INR-denominated, tradeable via IBKR India)
        "NIFTYBEES.NS":  {"Name": "Nippon India Nifty 50 BeES",     "Focus": "India Nifty 50"},
        "BANKBEES.NS":   {"Name": "Nippon India ETF Bank BeES",      "Focus": "India Banking"},
        "JUNIORBEES.NS": {"Name": "Nippon India ETF Junior BeES",    "Focus": "India Nifty Next 50"},
        "MON100.NS":     {"Name": "Motilal Oswal Nasdaq 100 ETF",    "Focus": "US Tech (INR)"},
        "ITBEES.NS":     {"Name": "Nippon India ETF Nifty IT",       "Focus": "India IT Sector"},
        "GOLDBEES.NS":   {"Name": "Nippon India ETF Gold BeES",      "Focus": "Gold (INR)"},
    }

    # Fallback values in case live fetch fails
    _ER_FALLBACK    = {
        "CSPX.L":0.07,"VUAA.L":0.07,"VWRA.L":0.22,"SWRD.L":0.12,
        "ISAC.L":0.20,"ANAU.DE":0.14,"XNAS.L":0.20,"2854.T":0.30,"1475.T":0.05,
        # India ETFs
        "INDA":0.65,"INDY":0.93,"SMIN":0.74,"EPI":0.84,
        "NIFTYBEES.NS":0.04,"BANKBEES.NS":0.19,"JUNIORBEES.NS":0.19,
        "MON100.NS":0.56,"ITBEES.NS":0.22,"GOLDBEES.NS":0.79,
    }
    _YIELD_FALLBACK = {
        "CSPX.L":1.10,"VUAA.L":1.15,"VWRA.L":1.50,"SWRD.L":1.41,
        "ISAC.L":1.45,"ANAU.DE":0.70,"XNAS.L":0.00,"2854.T":0.71,"1475.T":1.70,
        # India ETFs
        "INDA":0.20,"INDY":0.16,"SMIN":0.10,"EPI":0.60,
        "NIFTYBEES.NS":0.80,"BANKBEES.NS":0.30,"JUNIORBEES.NS":0.50,
        "MON100.NS":0.00,"ITBEES.NS":0.20,"GOLDBEES.NS":0.00,
    }
    ...
    def fetch_sg_etf_data():
        ...
        try:    inr_usd = yf.Ticker("INRUSD=X").fast_info["last_price"]
        except: inr_usd = 0.012   # fallback ~84 INR/USD
        ...
        for ticker, meta in SG_ETF_UNIVERSE.items():
            ...
            try:
                ...
                if ".T" in ticker:
                    ...
                    price_usd = curr_p * inr_usd   # INR → USD
                else:
                    price_usd = curr_p
...
```bash
pip install streamlit yfinance pandas numpy ta financedatabase plotly python-dotenv
```

### Run the scanner

```bash
python -m streamlit run swing_trader_v8.py
```
...
| Error | Fix |
|-------|-----|
| `AttributeError: set_page_config` | Run `python -m pip install --upgrade streamlit` then use `python -m streamlit run` |
| `Could not fetch sector data` | Markets may be closed (weekend/holiday). Data updates during market hours. |
| `No data for ticker` | Check ticker format. SGX stocks need `.SI` suffix (e.g. `D05.SI`). |
| `FinanceDatabase not installed` | Run `pip install financedatabase`. Scanner works without it but ETF holdings will be limited. |
| `Plotly chart error` | Run `pip install plotly`. Charts in Stock Analysis tab require plotly. |
| Scan takes too long | Reduce **Top N sectors** slider to 2. Or disable earnings guard to skip per-ticker API calls. |
| Empty results | Markets may be flat. Try lowering Min rise prob to 55%. |
...
Click **🔄 Refresh ETF Holdings** in the ETF tab to force a refresh.
    ...
    st.markdown("---")
    st.markdown("Create by Ripin")
    st.caption("Swing Scanner v8 · · Not financial advice")
